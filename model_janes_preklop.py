...
parser.add_argument("--label_scheme", type=str, default="binary_token",
                    choices=["binary_token", "multiclass_token"])

parser.add_argument("--train_path", type=str, default="janes_preklop-preprocessed-binary_token/fold0/train.jsonl")
parser.add_argument("--dev_path", type=str, default="janes_preklop-preprocessed-binary_token/fold0/dev.jsonl")
parser.add_argument("--test_path", type=str, default="janes_preklop-preprocessed-binary_token/fold0/test.jsonl")
...
if __name__ == "__main__":
    ...
    if args.label_scheme == "binary_token":
        uniq_labels = ["O", "B-code_switched", "I-code_switched"]
    else:
        uniq_labels = ["O", "B-en", "I-en", "B-de", "I-de", "B-hbs", "I-hbs", "B-sp", "I-sp", "B-la", "I-la",
                       "B-ar", "I-ar", "B-fr", "I-fr", "B-it", "I-it", "B-pt", "I-pt"]
    id2label = {_i: _lbl for _i, _lbl in enumerate(uniq_labels)}
    label2id = {_lbl: _i for _i, _lbl in id2label.items()}
    num_classes = len(id2label)

    train_data = train_data.to_pandas()
    train_data["mapped_language"] = train_data["mapped_language"].apply(lambda _ne_tags: list(map(lambda _lbl: label2id[_lbl], _ne_tags)))
    train_data = Dataset.from_pandas(train_data)

    dev_data = dev_data.to_pandas()
    dev_data["mapped_language"] = dev_data["mapped_language"].apply(lambda _ne_tags: list(map(lambda _lbl: label2id[_lbl], _ne_tags)))
    dev_data = Dataset.from_pandas(dev_data)

    test_data = test_data.to_pandas()
    test_data["mapped_language"] = test_data["mapped_language"].apply(lambda _ne_tags: list(map(lambda _lbl: label2id[_lbl], _ne_tags)))
    ...
    def tokenize_and_align_labels(examples):
        ...
        all_labels = examples["mapped_language"]
        new_labels = []
    ...
    for idx_ex in range(len(test_data)):
        ...
        assert len(test_word_preds[idx_ex]) == len(test_data["mapped_language"][idx_ex])

    correct_word_tags: List[List[str]] = list(
        map(lambda ne_tags: list(map(lambda _idx_class: id2label[_idx_class], ne_tags)), test_data["mapped_language"])
    )
